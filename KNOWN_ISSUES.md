...
| ~~`common-core-stateless-rvn`~~ | **NOT A DEFECT ON 7.40 — re-graded 2026-09-06** | `common_core.ts:80,122` | `rvn`/`commandRevision` are hard-coded to 1 and never persisted, and **7.40 cannot read either**. Binary scan, controls in the same run (`QueryProfile` PRESENT, `EOS_Initialize` ABSENT): `profileRevision`, `profileCommandRevision`, `responseVersion` and `commandRevision` are all ABSENT from the client; only `profileChangesBaseRevision` and `profileChanges` are present. Same finding that narrowed `mcp-rvn-from-client`, same method. Persisting a value this client provably ignores would risk the working baseline for no observable gain. Revisit only for a build that reads those fields. |
...
| ~~`stale-dist-preferred`~~ | **NOT A DEFECT — re-graded 2026-09-06** | `main.rs:156-168` | `start_backend` preferring `dist/` over sources is **correct**: `dist/` is the shipped artifact, and running sources instead would mean an installed launcher executing something that was never staged or verified. The real defect was always that `dist/` could go stale silently — that is now gated by `node tools/stage-backend.mjs --check`, which is in RELEASING.md as a pre-release requirement alongside the two native ones. Keeping this open implies a fix that would make things worse. |
...
| ~~`dependency-hygiene`~~ | **FIXED 2026-09-06** | `Launcher/src-tauri/Cargo.toml`, `Launcher/package.json` | Removed. `mongodb` had 0 usages; `electron` had 0 imports; `warp` existed only for `impl warp::reject::Reject for MyError`, and `MyError` was constructed nowhere — a dead type whose sole effect was making an entire web framework a build dependency of a desktop launcher that serves no HTTP. The type and both its impls are gone too. **65 crates dropped from the dependency graph (707 → 642).** `cargo check` and the frontend build both clean. |
