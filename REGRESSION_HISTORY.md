...
## NOVA-AUDIT-016 — every build was served Chapter 1's season

| | |
|---|---|
| **Date** | 2026-09-05 · **Severity** P2 · **Subsystem** version / content |
| **Grade** | CONFIRMED — measured across five builds against the real handlers |

**Symptom.** None visible on 7.40, which is the only build anyone runs. It would have appeared later
as "Chapter 2 support is mysteriously broken".

**Root cause — two of them, in opposite directions.**

1. `version-router.ts` set `season = major`, commented "In Ch1, major version = season number". True,
   and true only in Chapter 1: from 11.00 the major keeps counting while the season restarts each
   chapter. Every consumer of `gameVersion.season` was wrong by construction outside Chapter 1.
2. `content/api/pages/fortnite-game` read `Config.SEASON_NUMBER` — a static value — so the lobby
   background was `lobbyseason7` for **every** build. The file's own comment records the same bug from
   the other side: hardcoding season 8 once gave a 7.40 client a stage it had no assets for. Moving to
   Config fixed 7.40 and reintroduced it everywhere else.

**The trap in fixing (1).** Making `season` correct is not automatically safe for its readers, and one
would have broken. Epic's timeline uses the **continuous major** for `seasonNumber` and
`athenaseason<n>`: `Calendar.md` shows `seasonNumber: 24` for a document dated May 2023, and the build
registry — an entirely independent source — places major 24 at Chapter 4 Season 2. A chapter-relative
value would have told a Chapter 4 client it was in Chapter 1 Season 2. In Chapter 1 the two numbers
coincide, which is exactly why 7.40 could never reveal it.

So wire formats read `major`; `chapter`/`season` are for human-facing reasoning. `cloudstorage` keys
`ClientSettings-<n>.Sav` on major for the same reason — on `season`, Chapter 1 Season 7 and Chapter 2
Season 7 would collide on one file — and because the old code set `season = major`, that rename is a
no-op on disk.

**How (2) was found.** Not by the tests, which passed. By diffing whole responses across five
User-Agents after the version model landed: the endpoint reported as version-varying because *other*
fields varied while this one silently did not. The new test asserts per field.

**Regression test.** `version.test.ts` — a five-case matrix across Chapters 1-4 asserting seasonNumber,
`athenaseason<n>`, the season flag and the lobby stage per build, plus that Chapter 1's era-specific
event flags do not leak into a Chapter 2 build, plus that `versioncheck` / `enabled_features` /
`lightswitch` stay **identical** across all five.

---

## NOVA-AUDIT-017 — the keychain served one build's keys to every build

| | |
|---|---|
| **Date** | 2026-09-05 · **Severity** P3 · **Subsystem** content / storefront |
| **Grade** | CONFIRMED — the archive lists the keys; 7.40's two were verified by hand |

**Symptom.** None on 7.40, for which the answer was correct. Any other build would have received keys
for chunks it does not have.

**Root cause.** `GET /fortnite/api/storefront/v2/keychain` returned two hard-coded 7.40 chunk keys.
Chunk keys are per-build and do not carry forward.

**Fix.** `version/keychain.ts`, generated by `tools/aes-registry.mjs` from the
Fortnite-Aes-Keys-Archive corpus — **75 builds, 634 keys**, Chapters 1-3 — and served by exact build
match. An unknown build gets an empty array rather than someone else's keys: a wrong key presents as a
decryption bug, which is harder to diagnose than a missing one.

**66 chunks the archive records as `???` are omitted and counted, never invented.** The generator
refuses to write the file if 7.40 stops yielding exactly its two verified keys, so a broken parse
fails loudly instead of quietly shipping fewer.

**Regression test.** Seven, including the one that matters — a different build must return
*different* keys — plus a check that every entry is a well-formed GUID and a 32-byte key.
...
```bash
cd "Main backend"        && npm test        # 143 tests · node:test via tsx
cd "Main backend"        && npm run typecheck
cd Launcher/src-tauri    && cargo test --bin sololauncher   # 17 tests
Launcher/cobalt/Cobalt/tests/build-and-run.cmd               # 11 C++ redaction cases + drift check
