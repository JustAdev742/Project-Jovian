...
void Inventory::Update(UObject* Controller, bool bAddOrRemove, FFastArraySerializerItem* ModifiedItem) // TODO: add eFortquickbars bars and thenm update accoridng to that
{
	...
	// Guard both: a null WorldInventory (or a not-found function) here dereferences null inside
	// ProcessEvent (the 0x98 read seen in the crash) instead of just no-op'ing the update.
	if (WorldInventory && WorldHandleInvUpdate)
		WorldInventory->ProcessEvent(WorldHandleInvUpdate); // This like sets the OwnerInventory probs idfk

	// 7.40 uses the AFortQuickBars actor path (OnRep the quickbars so the client rebuilds slots from
	// the server-mapped GUIDs). `< 7.41` includes 7.40; the old `< 7.4` sent it down the
	// ClientForceUpdateQuickbar branch, which has no server slot mapping -> GUIDs unresolved.
	...
	if (ItemInstance)
	{
			...
			if (InstancesOfItem.size() > 0)
			{
				...
				for (auto InstanceOfItem : InstancesOfItem)
				{
					if (InstanceOfItem)
					{
						...
						// Must check the EXISTING instance's count (CurrentItemEntry), not the incoming
						// item's (ItemEntry). Reading ItemEntry made the loop grab the FIRST existing
						// stack even if it was full, so a partial stack was never preferred and overflow
						// was mishandled. Now it finds a genuinely non-full stack to merge into.
						if (CurrentItemEntry)
						{
							auto currentCount = FFortItemEntry::GetCount(CurrentItemEntry);
		...
		auto CreateAndAddItem = [&](int countForItem) {
			...
			*FFortItemEntry::GetCount(ItemEntry) = countForItem;
			...
			// 7.40 uses the AFortQuickBars actor (see the note at the actor spawn) — map this item's
			// GUID to its quickbar slot so the client's AddItemInternal can resolve it. `< 7.41`
			// includes 7.40; the old `< 7.4` skipped it, leaving quickbar slots unmapped.
			if (Fortnite_Version < 7.41)
			{
