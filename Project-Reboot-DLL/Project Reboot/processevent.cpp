...
{
	Sleep((DWORD)Defines::SecondsBeforeRestart * 1000);

	std::cout << "[Restart] recycling the server for a new match\n";
...
bool OnGamePhaseChanged(UObject* MatchAnaylitics, UFunction*, void* Parameters)
{
	...
	if (Phase == EAthenaGamePhase::EndGame && Defines::bAutoRestartAfterMatch && !Defines::bIsRestarting)
	{
		...
		{
			bRestartQueued = true;
			std::cout << "[Restart] match over - recycling in " << Defines::SecondsBeforeRestart << "s\n";
