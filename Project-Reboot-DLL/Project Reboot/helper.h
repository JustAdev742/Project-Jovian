...
namespace Helper
{
	...
	UObject* FindCIDByName(const std::string& Name);
	UObject* GetEquippedCID(UObject* Controller);

	// The player's full equipped loadout, resolved to loaded objects. Any field may be null (that
	// slot is a default or the player is unknown to the backend).
	struct EquippedLoadout
	{
		UObject* Character = nullptr;  // AthenaCharacterItemDefinition (CID)
		UObject* Backpack = nullptr;   // AthenaBackpackItemDefinition   (BID)
		UObject* Pickaxe = nullptr;    // AthenaPickaxeItemDefinition
	};

	EquippedLoadout GetEquippedLoadout(UObject* Controller);
	bool ApplyBackpack(UObject* Pawn, UObject* BackpackDef);
