...
namespace Defines
{
	...
	// WARMUP FILL TRACKING (instrumentation only -- it does NOT shorten the hold).
	//
	...
	//
	// How long after the last arrival the lobby is considered settled. Logged, not acted on -- see
	// the long note in server.cpp for why ending the hold on this made the wait WORSE for the
	// player rather than better.
