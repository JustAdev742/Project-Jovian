...
function Settings() {
  ...
  useEffect(() => {
    ...
    // Read the SAME key the toggle writes ("EOR") — it previously read a different key
    // ("autoLaunchEnabled"), so the setting never persisted across launches.
    const raw = localStorage.getItem("EOR");
    if (raw !== null) {
      setEOR(raw === "true");
    }
  }, []);
      ...
      <div className="h-full w-full flex">
        ...
        >
          ...
          <div className="p-3 space-y-2">
            ...
            <div className="pt-4" />
            {/* Settings is the current page — mark it active rather than leaving a dead, clickable-looking control. */}
            <NavItem icon={SettingsIcon} label="Settings" active collapsed={collapsed} />
          </div>
