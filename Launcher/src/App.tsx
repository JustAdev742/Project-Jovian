...
function App() {
  ...
  useEffect(() => {
    ...
    // Validate the stored session BEFORE trusting it — the previous version navigated
    // to /onboard immediately, so a corrupt/incomplete "user" was never re-checked.
    try {
      const parsed = JSON.parse(raw);
      const hasEmail = parsed && typeof parsed.email === "string" && parsed.email.length > 0;
      const hasToken = parsed && typeof parsed.token === "string" && parsed.token.length > 0;
      const hasPassword = parsed && typeof parsed.password === "string" && parsed.password.length > 0;
      if (!hasEmail || (!hasToken && !hasPassword)) {
