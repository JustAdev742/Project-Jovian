...
bool Helper::ApplyCID(UObject* Pawn, UObject* CID)
{
	...
	for (int j = 0; j < HeroSpecializations->Num(); j++)
	{
		...
		for (int i = 0; i < CharacterParts->Num(); i++)
		{
			...
			if (CharacterPart)
			{
				// Per part, not `static` — see ApplyBackpack for why caching this offset across
				// different assets produces a garbage part type rather than an error.
				int CharacterPartTypeOffset = CharacterPart->GetOffset("CharacterPartType");
				if (!CharacterPartTypeOffset)
				{
					// GetOffset returns 0 for "not found", and offset 0 on a UObject is the low byte of
					// its vtable pointer — which lands inside the valid 0-5 enum range often enough to
					// silently attach a part to the wrong slot rather than fail.
					std::cout << "[Locker] part '" << CharacterPart->GetName() << "' has no CharacterPartType - skipping" << std::endl;
					continue;
				}
...
/**
 * Apply a backpack's character parts (the back bling mesh) to the pawn.
 *
 * NOT "the same mechanism as ApplyCID", which is what the previous version assumed and what made it
 * crash the server every single time a player with a back bling spawned.
 *
 * Both classes have a property called CharacterParts, but they are different types:
 *
 *   FortHeroSpecialization::CharacterParts        TArray<TSoftObjectPtr<UCustomCharacterPart>>  (0x28/elem)
 *   AthenaCharacterPartItemDefinition::CharacterParts  TArray<UCustomCharacterPart*>            (0x08/elem)
 *
 * (Back bling inherits the second from AthenaCharacterPartItemDefinition; AthenaBackpackItemDefinition
 * itself declares nothing.) Reading the item definition's array as soft pointers strides FIVE TIMES
 * too far and reads AssetPathName at +0x10 inside an 8-byte element — past the end of the allocation.
 * That garbage becomes an FName, FName::ToString() hands it to the engine, and the engine indexes its
 * global name table with it: an access violation deep inside Fortnite, with our name on it.
 *
 * No amount of validating the TArray header could have caught that — both instantiations share the
 * identical { Data, ArrayNum, ArrayMax } header, so every field reads perfectly sane. The type was
 * the bug. The parts are hard references and already loaded, so there is nothing to resolve: take
 * the pointer and use it.
 */
bool Helper::ApplyBackpack(UObject* Pawn, UObject* BackpackDef)
{
	...
	{
		std::cout << "[Locker] backpack '" << BackpackDef->GetName() << "' has no CharacterParts property" << std::endl;
		return false;
	}

	auto& CharacterParts = *(TArray<UObject*>*)(__int64(BackpackDef) + CharacterPartsOffset);
	const int PartCount = CharacterParts.Num();

	if (PartCount <= 0)
	{
		std::cout << "[Locker] backpack '" << BackpackDef->GetName() << "' has no character parts" << std::endl;
		return false;
	}
	...
	{
		auto Part = CharacterParts.At(i);
		if (!Part)
			continue;

		// Hardcoded rather than read back off the part. This IS the backpack slot by definition, and
		// the type byte was only ever consulted because the old code was modelled on the character
		// path, where one array carries head/body/hat parts together.
		Helper::ChoosePart(Pawn, EFortCustomPartType::Backpack, Part);
		bApplied = true;
	}
...
UObject* Helper::SpawnPawn(UObject* Controller, BothVector Location, bool bAssignCharacterParts)
{
	...
	if (bAssignCharacterParts)
	{
		...
		if (Loadout.Backpack)
		{
			// std::endl, not '\n': stdout is a pipe here (Cobalt forwards it to the launcher), so it
			// is fully buffered and a hard crash discards whatever is still in the buffer. That is why
			// earlier crash reports showed this line and then nothing — the diagnostics that would
			// have explained it were written, then thrown away. Flush anything that precedes a risky
			// read, or it tells you nothing exactly when you need it.
			std::cout << "[Locker] applying backpack: " << Loadout.Backpack->GetName() << std::endl;
			if (!ApplyBackpack(Pawn, Loadout.Backpack))
				std::cout << "[Locker] backpack apply FAILED" << std::endl;
			else
				std::cout << "[Locker] backpack applied OK" << std::endl;
		}
	}
...
void Helper::ChoosePart(UObject* Pawn, TEnumAsByte<EFortCustomPartType> Part, UObject* ChosenCharacterPart)
{
	...
	else */
	{
		...
		// Every locker path funnels through here, so this is the one place worth making unkillable.
		if (!Pawn || !ServerChoosePart || !ChosenCharacterPart)
		{
			std::cout << "[Locker] ChoosePart skipped (pawn=" << (void*)Pawn
			          << " fn=" << (void*)ServerChoosePart
			          << " part=" << (void*)ChosenCharacterPart << ")\n";
			return;
		}

		// The part type is read out of the asset at a looked-up offset, and a wrong offset yields a
		// garbage byte rather than an error. ServerChoosePart uses it to index a fixed Parts[] array
		// on the player state, so a value like 127 is an out-of-bounds WRITE inside the game — which
		// is exactly how applying a back bling was taking the whole server down (0xC0000005 at a
		// Fortnite address, right after "[Locker] applying backpack"). Anything outside the real enum
		// is not a part type we can honour, so drop it rather than corrupt memory with it.
		if ((uint8_t)Part.Get() >= (uint8_t)EFortCustomPartType::NumTypes)
		{
			std::cout << "[Locker] ChoosePart skipped - implausible part type " << (int)Part.Get()
			          << " for " << ChosenCharacterPart->GetName() << '\n';
			return;
		}
...
UObject* Helper::GetPlayerStart()
{
	...
	// Re-query every time rather than caching for the process lifetime. These are level actors: they
	// do not exist until the sublevel holding them has streamed in, so a cached empty result would
	// stay empty forever — and a cached NON-empty one would be dangling pointers after a restart.
	auto OutActors = GetAllActorsOfClass(WarmupClass);

	const int Num = OutActors.Num();
	if (Num <= 0)
		return nullptr;

	// The old index was `GetRandomFloat(2, Num - 1)`, which is undefined when Num <= 2 (min > max),
	// and TArray::At is not bounds-checked — so with one or two spawn points this read past the end
	// of the array and returned non-null GARBAGE, which then became a spawn location. It also made
	// entries 0 and 1 unreachable for no reason.
	for (int attempt = 0; attempt < 8; attempt++)
	{
		const int Index = Num == 1 ? 0 : (int)std::floor(GetRandomFloat(0, (float)(Num - 1)));
		if (Index < 0 || Index >= Num)
			continue;
		if (auto ActorToUse = OutActors.At(Index))
			return ActorToUse;
	}

	// Every pick came back null — take the first non-null rather than spinning forever, which is what
	// the old `while (!ActorToUse)` loop did with no way out.
	for (int i = 0; i < Num; i++)
	{
		if (auto ActorToUse = OutActors.At(i))
			return ActorToUse;
	}
