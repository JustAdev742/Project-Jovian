...
ObjectType* FindObject(const std::string& ObjectName, UObject* Class, UObject* InOuter)
{
	...
	// The wstring must OUTLIVE the call. Written as `std::wstring(...).c_str()` the temporary died at
	// the end of that statement and the game was handed a dangling pointer. MSVC's small-string buffer
	// holds 7 wchar_t, so any name longer than that is heap-allocated and freed before use — which is
	// every name that matters here ("CharacterParts" is 14). It only appeared to work because the
	// freed block usually still held its old contents.
	auto ObjectNameWide = std::wstring(ObjectNameCut.begin(), ObjectNameCut.end());

	return (ObjectType*)StaticFindObjectO(Class, InOuter, ObjectNameWide.c_str(), false);
}
