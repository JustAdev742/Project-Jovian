...
inline uint64_t SendRequestNowAddress = 0;
...
static bool InitializePatterns()
{
	...
	// MCP-Profile::SendRequestNow — see Server::Hooks::SendRequestNow. A dedicated server dispatches
	// its profile requests with a credential context it doesn't have, so the request never fires and
	// GetAthenaLoadoutWithOverrides() can never load a player's AthenaProfile. Hooking this lets us
	// force the context to CXC_Public. Located by the log string inside the function, then walking
	// backwards to the prologue. (Technique from Erbium's FindSendRequestNow.)
	{
		auto StringRef = Memcury::Scanner::FindStringRef(L"MCP-Profile: Dispatching request to %s");

		if (!StringRef.Get())
			StringRef = Memcury::Scanner::FindStringRef(L"MCP-Profile: Dispatching request to %s - ContextCredentials: %s");

		if (auto Addr = StringRef.Get())
		{
			for (int i = 0; i < 1000; i++)
			{
				auto Ptr = (uint8_t*)(Addr - i);

				// mov [rsp+x], rbx   /   mov rax, rsp   — the two prologues this function compiles to.
				if ((Ptr[0] == 0x48 && Ptr[1] == 0x89 && Ptr[2] == 0x5C) ||
					(Ptr[0] == 0x48 && Ptr[1] == 0x8B && Ptr[2] == 0xC4))
				{
					SendRequestNowAddress = uint64_t(Ptr);
					break;
				}
			}
		}
