...
static T* StaticLoadObject(UObject* Class, UObject* Outer, const std::string& name, int LoadFlags = 0)
{
	...
	if (!Object)
	{
		// Named local, not a temporary — see FindObject in structs.cpp. Asset paths are always well
		// past the 7-character small-string buffer, so this one ALWAYS handed the game freed memory.
		auto Name = std::wstring(name.begin(), name.end());
		Object = (T*)StaticLoadObjectO(Class, Outer, Name.c_str(), nullptr, LoadFlags, nullptr, false, nullptr);
	}
...
// [Nova] Real serial-number lookup by object index (the original code called an undefined
// GetSerialNumber(GetByIndex<UObject>(...)); the newer MSVC conformance mode parses that
// template body eagerly and rejects the missing identifiers).
static int32_t GetSerialNumberByIndex(int Index) { auto Item = OldObjects ? OldObjects->GetItemById(Index) : NewObjects->GetItemById(Index); return Item ? Item->SerialNumber : 0; }
...
struct FGameplayTag
{
	...
	bool operator==(const FGameplayTag& OtherTag) const { return TagName.ComparisonIndex == OtherTag.TagName.ComparisonIndex;  } // [Nova] const: C++20 C2666 fix
};
...
public:
	...
	TWeakObjectPtr(int32_t ObjectIndex)
	{
		...
		this->ObjectSerialNumber = GetSerialNumberByIndex(ObjectIndex);
	}

	TWeakObjectPtr(UObject* Obj)
	{
		this->ObjectIndex = Obj->InternalIndex;
		this->ObjectSerialNumber = GetSerialNumberByIndex(Obj->InternalIndex);
	}
