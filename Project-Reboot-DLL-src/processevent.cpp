...
bool HandleStartingNewPlayer(UObject* Object, UFunction* Function, void* Parameters)
{
	...
	// RETURN TRUE FROM EVERY PATH BELOW THIS LINE.
	//
	// The native has just been run, on the line above. Returning false tells the dispatcher we did not
	// handle it, so it runs the native a SECOND time — and this native is HandleStartingNewPlayer,
	// which goes on to RestartPlayer -> SpawnDefaultPawnFor -> Possess. Possessing twice makes the
	// client drop to a loading screen and come back, which is exactly the flicker being reported.
	if (!Parameters) // possible?
		return true;

	// REMOVED: a `static int LastTime = 931249515401;` compared against Defines::AmountOfRestarts,
	// which returned early the first time it ran. Because AmountOfRestarts is 0 at the start of a
	// match and that initialiser is not, the VERY FIRST PLAYER always took that early return — so
	// player one alone never got QuickBars created and never had bHasServerFinishedLoading set. That
	// flag is what tells their client it may stop showing the loading screen, and the missing quickbar
	// is the "Failed to find3 QuickBars" / unresolvable item GUIDs in the log. In a one-player test
	// nobody was ever set up at all, because the only player was always player one.
	...
	if (PlayerController)
	{
		if (Moderation::Banning::IsBanned(PlayerController))
		{
			...
			return true; // native already ran at the top — see the note there
		}
		...
		// 7.40 (Chapter 1 S7) STILL uses the AFortQuickBars actor — the client error
		// "AFortQuickBars::AddItemInternal could not find Item with GUID" and MoonGS 7.30 (identical
		// engine) both prove it. The old `< 7.4` gate excluded 7.40 (7.4 < 7.4 is false), so the server
		// never created a quickbar nor mapped item GUIDs to slots -> the client couldn't resolve them.
		// `< 7.41` includes 7.40 and below. Null-guard the spawn so we don't replace a native one.
		if (Fortnite_Version < 7.41)
		{
			...
			auto& QuickBarsRef = *(UObject**)(__int64(PlayerController) + QuickBarsOffset);
			if (!QuickBarsRef)
				QuickBarsRef = Helper::Easy::SpawnActor(QuickBarsClass, FVector(), FRotator(), PlayerController);
		}
...
bool ServerReadyToStartMatch(UObject* PlayerController, UFunction* Function, void* Parameters)
{
	...
	// Give the player's EQUIPPED pickaxe, not a random one. GetEquippedLoadout is cached, so this is
	// a cheap map lookup; the pickaxe item def holds the actual harvesting WID in WeaponDefinition.
	UObject* PickaxeDef = nullptr;

	if (auto ChosenPickaxe = Helper::GetEquippedLoadout(PlayerController).Pickaxe)
	{
		...
		std::cout << "[Locker] applying equipped pickaxe: " << ChosenPickaxe->GetName() << '\n';
	}

	if (!PickaxeDef) // unknown player / default slot — keep stock behaviour
		PickaxeDef = Helper::GetPickaxeDef(PlayerController, true);
	...
	if (bFirst)
	{
		...
		// A lobby must still be able to start with a single player once the warmup hold expires --
		// otherwise holding for 90s just means a solo player waits 90s and then nothing happens.
		// (Technique from Erbium, which sets this unconditionally; the property exists on 7.40.)
		static auto WarmupRequiredPlayerCountOffset = GameMode->GetOffset("WarmupRequiredPlayerCount");

		if (WarmupRequiredPlayerCountOffset)
			*Get<int>(GameMode, WarmupRequiredPlayerCountOffset) = 1;
		...
		if (bGoIntoWarmup)
		{
			...
			*Get<EAthenaGamePhase>(GameState, GamePhaseOffset) = EAthenaGamePhase::Warmup;
			...
			// Hold the lobby open (see Defines::WarmupWaitSeconds) instead of letting the playlist's
			// ~14s default launch the bus on top of the first player to load in.
			static auto WarmupCountdownEndTimeOffset = GameState->GetOffset("WarmupCountdownEndTime");
			static auto WarmupCountdownStartTimeOffset = GameState->GetOffset("WarmupCountdownStartTime");

			std::cout << "[Warmup] offsets: EndTime=" << WarmupCountdownEndTimeOffset
			          << " StartTime=" << WarmupCountdownStartTimeOffset << '\n';

			if (WarmupCountdownEndTimeOffset)
			{
				auto Now = Helper::GetTimeSeconds();

				if (WarmupCountdownStartTimeOffset)
				...
				*Get<float>(GameState, WarmupCountdownEndTimeOffset) = Defines::WarmupHoldUntil;
				...
				// MOVE THE STORM TO WHERE THE MATCH ACTUALLY BEGINS.
				//
				// StartMatch a few lines above starts the engine's match clock, and with it the native
				// safe-zone schedule. We then force the phase back to Warmup and hold the lobby for
				// WarmupWaitSeconds — but nothing moves the storm by that same amount, so the zone
				// spends the entire warmup advancing without the players. They land into a schedule
				// that is already a minute and a half in, and start taking OutsideSafeZone damage while
				// their HUD still draws the opening circle: standing inside the circle and dying to the
				// storm anyway. Pinning the start to the bus makes the two agree again.
				static auto SafeZonesStartTimeOffset = GameState->GetOffset("SafeZonesStartTime", false, false, false);

				if (SafeZonesStartTimeOffset)
				{
					*Get<float>(GameState, SafeZonesStartTimeOffset) = Defines::WarmupHoldUntil;
					std::cout << "[Warmup] storm schedule starts with the bus (world time "
					          << Defines::WarmupHoldUntil << ")" << std::endl;
				}
				else
				{
					std::cout << "[Warmup] SafeZonesStartTime not found - the storm will run "
					          << Defines::WarmupWaitSeconds << "s ahead of the players" << std::endl;
				}

				// Make the countdown VISIBLE for the whole hold. The HUD only draws numbers once the
				// remaining time drops inside WarmupCountdownDuration -- stock that is ~10-15s, so a
				// 90s hold spends 75s showing nothing and looks broken. Widening it to the full hold
				// means the timer is on screen the entire time.
				if (auto WarmupCountdownDurationOffset = GameMode->GetOffset("WarmupCountdownDuration", false, false, false))
				{
					*Get<float>(GameMode, WarmupCountdownDurationOffset) = Defines::WarmupWaitSeconds;
					std::cout << "[Warmup] countdown visible for the full " << Defines::WarmupWaitSeconds << "s\n";
				}
				else
				{
					std::cout << "[Warmup] WarmupCountdownDuration not on the GameMode - the HUD will "
					             "only show numbers near the end\n";
				}

				std::cout << "[Warmup] holding the lobby open for " << Defines::WarmupWaitSeconds
				          << "s so players can join (bus at world time " << Defines::WarmupHoldUntil << ")\n";
			}
			else
			{
				std::cout << "[Warmup] WarmupCountdownEndTime offset not found - using the playlist default\n";
			}
		}
	}
...
bool ReadyToStartMatch(UObject* GameMode, UFunction* Function, void* Parameters)
{
	...
	if (Defines::bReadyForStartMatch)
	{
		...
		*Get<EAthenaGamePhase>(GameState, GamePhaseOffset) = EAthenaGamePhase::None;
		...
		// Dedicated server drops its local player (the host is not a participant). A listen server
		// KEEPS it so the host spawns a pawn and plays.
		if (LocalPlayers.size() && !Defines::bHostCanPlay)
...
bool ClientOnPawnDied(UObject* DeadController, UFunction* fn, void* Parameters)
{
	...
	// Everything below dereferences this. A player who LEAVES arrives here as a death
	// (DeathCause.LoggedOut) with their PlayerState potentially already gone, so bail rather than
	// walk a null pointer through the whole death report.
	if (!DeadPlayerState)
	{
		std::cout << "[Death] no playerstate (player already gone) - skipping the death report\n";
		return true;
	}
	...
	*(bool*)(__int64(DeathInfo) + preoffsets::bDBNO) = bWasDBNO;
	...
	if (preoffsets::Distance != 0)
	{
		...
		{
			// GetDistanceTo runs ProcessEvent against KillerPawn with DeadPawn as the other actor; a
			// null DeadPawn (player left, pawn already gone) is an access violation, so require both.
			*(float*)(__int64(DeathInfo) + preoffsets::Distance) = (KillerPawn && DeadPawn) ? Helper::GetDistanceTo(KillerPawn, DeadPawn) : 0.f;
		}
		else
		{
			static auto LastFallDistanceOffset = preoffsets::LastFallDistance; // DeadPawn->GetOffsetSlow("LastFallDistance");

			if (LastFallDistanceOffset != -1 && DeadPawn)
	...
	// Offset 0 means the lookup FAILED — and offset 0 on a UObject is its vtable pointer. Writing
	// "false" there corrupts the object, and reading it back gives a nonzero byte that reads as
	// "alive", which would make a dead player's team look like it still has survivors. Say so loudly
	// rather than quietly scribbling on a vtable.
	if (!bMarkedAliveOffset)
	{
		std::cout << "[Death] bMarkedAlive offset not found - skipping alive tracking (would corrupt "
		             "the controller). Team counts this match are unreliable." << std::endl;
	}
	else
	{
	...
	if (!Defines::bIsPlayground) // && OldPhase > EAthenaGamePhase::Warmup)
	{
		...
		// *PlayerTeam can be null (team torn down / never assigned); dereferencing it for the offset
		// and the members array would fault. Skip the team-elimination accounting if so.
		if (TeamsLeftOffset != 0 && PlayerTeam && *PlayerTeam)
		{
			...
			for (int i = 0; i < TeamMembers->Num(); i++)
			{
				...
				// Without a real offset this reads the low byte of the vtable pointer, which is
				// essentially always nonzero — i.e. every team member reads as alive forever.
				auto bAlive = bMarkedAliveOffset ? *Get<bool>(TeamMember, bMarkedAliveOffset) : false;
				std::cout << std::format("[{}] bAlive: {} (offset {})", i, bAlive, bMarkedAliveOffset);
	...
	// A player LEAVING reaches this path as a death (DeathCause.LoggedOut) while their PlayerState is
	// already being torn down. Writing Place unguarded is then either a null deref or — worse, if
	// the offset failed to resolve — a write to PlayerState+0, straight through the object header.
	// That is where the server was dying as the last player left.
	static auto PlaceOffset = preoffsets::Place;

	// The placement this player finished in. Tracked as a value so the match-report code below
	// doesn't have to re-read a pointer that may not have been safe to write in the first place.
	int DeadPlace = TeamsLeftBefore;

	if (PlaceOffset != 0)
	{
		*Get<int>(DeadPlayerState, PlaceOffset) = DeadPlace;
	}
	else
	{
		std::cout << "[Death] Place offset unresolved - not writing it\n";
	}

	std::cout << "[Death] place ok\n";

	// A voluntary LEAVE arrives here as a death with cause "LoggedOut". It must NOT be treated as the
	// match ending: firing EndMatch + the Victory-Royale fanfare (and the auto-restart that follows
	// on EndGame) tears the server down UNDER the departing client, which the client then shows as
	// "There was a problem — we lost our connection to the match." An actual elimination is unchanged.
	static auto LoggedOutDeathCause = GetEnumValue(FindObject("/Script/FortniteGame.EDeathCause"), "LoggedOut");
	bool bWasLogout = (DeathCause == LoggedOutDeathCause);

	if (bWasLogout)
		std::cout << "[Death] player left (LoggedOut) - not ending the match, letting them go cleanly\n";

	if (GamePhase > EAthenaGamePhase::Warmup)
	{
		// End the match on the deciding elimination — INCLUDING when the runner-up LEAVES and one
		// player is left standing (they should get the Victory Royale, and EndMatch is what drives
		// EndGame -> the auto-restart). Only suppress it when the LAST player themselves left
		// (PlayersLeft hit 0): firing EndMatch there is what tore the server down under the departing
		// client. A plain `!bWasLogout` skipped BOTH cases, so a runner-up quit hung the match.
		// if (*TeamsLeft <= 1) // && (int)Playlist->WinCondition <= 1
		if (*PlayersLeftPtr <= 1 && !Defines::bIsPlayground && !(bWasLogout && *PlayersLeftPtr <= 0))
		{
	...
	// DeadPawn can already be gone here (a LEAVE reaches this handler as a death AFTER the pawn is
	// torn down). GetActorLocation runs ProcessEvent on the actor, so a null pawn is an access
	// violation — fall back to a zero location. The loot drop below is itself skipped without a pawn.
	auto DeathLocation = DeadPawn ? Helper::GetActorLocation(DeadPawn) : FVector{};
	...
	// Drop loot only if we still have a pawn to drop it from; SummonPickup(DeadPawn=null, ...) faults,
	// and a location of {0,0,0} (no pawn) would dump everything at world origin anyway.
	if (!Defines::bIsPlayground && DeadPawn)
	{
...
bool ServerAttemptAircraftJump(UObject* Controller, UFunction*, void* Parameters)
{
	...
	// Re-apply the DefaultPlayer ability set to the freshly-spawned skydive pawn. SpawnPawn makes a
	// NEW pawn (new AbilitySystemComponent) but grants no abilities; only ServerReadyToStartMatch
	// grants the set, and that ran for the FIRST (warmup) pawn. Without this the pawn you land with has
	// an empty ASC — mantle/vault and the other default-player abilities are dead. MoonGS 7.30
	// re-applies it the same way after the bus jump.
	if (Pawn)
	{
		if (Fortnite_Version < 8.30)
		{
			static auto AbilitySet = FindObject("/Game/Abilities/Player/Generic/Traits/DefaultPlayer/GAS_DefaultPlayer.GAS_DefaultPlayer");
			if (AbilitySet) GiveFortAbilitySet(Pawn, AbilitySet);
		}
		else
		{
			static auto AbilitySet = FindObject("/Game/Abilities/Player/Generic/Traits/DefaultPlayer/GAS_AthenaPlayer.GAS_AthenaPlayer");
...
/**
 * Recycle the server into a fresh match once the current one ends.
 *
 * Runs on its own thread for two reasons: the delay must not block the game thread, and
 * Server::Restart() disables the TickFlush hook — calling that from inside TickFlush would be
 * disabling a hook while executing inside it. The debug GUI already calls Restart() off-thread,
 * so this is the same path that was known to work, just triggered automatically.
 *
 * The watchdog matters as much as the restart: a half-restarted server is WORSE than a dead one,
 * because the launcher keeps heart-beating it as healthy and players get routed into a match that
 * will never start. If the recycle hasn't completed in time we exit, which the launcher already
 * handles by unregistering and hosting fresh.
 */
static DWORD WINAPI MatchRestartThread(LPVOID)
{
	// Wait for the GAME THREAD to say recycling is safe, rather than sleeping a fixed period and
	// terminating on top of whoever is still connected. TickMatchEndWait sets this the moment the last
	// player disconnects — so leaving the results screen shuts the server down promptly — or after the
	// backstop if they never do. The cap is only here so a wedged game thread cannot hang us forever.
	const ULONGLONG Cap = GetTickCount64() + (ULONGLONG)(Defines::SecondsToWaitForPlayersToLeave + 30) * 1000ull;

	while (!Defines::bSafeToRecycle && GetTickCount64() < Cap)
		Sleep(500);

	if (!Defines::bSafeToRecycle)
		std::cout << "[Restart] gave up waiting for players to leave - recycling anyway" << std::endl;

	std::cout << "[Restart] recycling the server for a new match" << std::endl;
	...
	{
		std::cout << "[Restart] recycle did not finish within 120s - exiting so the launcher "
		             "can unregister this server and start a clean one\n";
		std::cout.flush();

		// TerminateProcess raises no exception, so the crash handler never runs — leave a breadcrumb
		// on disk so a stalled recycle is distinguishable from an actual fault.
		if (std::ofstream watchdog{ "crash.log", std::ios::app })
			watchdog << "\n[Restart watchdog] process terminated: match-end recycle stalled >120s "
			            "(intentional exit, not a fault)\n";

		TerminateProcess(GetCurrentProcess(), 1);
	}
	else
	{
		std::cout << "[Restart] server is live again and accepting players\n";
	}
...
bool OnGamePhaseChanged(UObject* MatchAnaylitics, UFunction*, void* Parameters)
{
	...
	if (Phase == EAthenaGamePhase::EndGame && Defines::bAutoRestartAfterMatch && !Defines::bIsRestarting)
	{
		static bool bRestartQueued = false;

		// EndGame can fire more than once; only ever queue one recycle.
		if (!bRestartQueued)
		{
			bRestartQueued = true;
			std::cout << "[Restart] match over - shutting down once everyone has left" << std::endl;

			// From here the game thread (TickMatchEndWait) watches for the last player to disconnect.
			// We do NOT pick a fixed moment to die: whoever won is still looking at the results screen,
			// and killing the process out from under them is what produced the connection error.
			Defines::MatchEndedAt = GetTickCount64();
			Defines::bSafeToRecycle = false;

			CreateThread(nullptr, 0, MatchRestartThread, nullptr, 0, nullptr);
		}
	}
			...
			if (RootComp)
			{
				...
				if (SetWorldTransform_Params && SetRelativeLocation_Params)
				{
					...
					*(FVector*)(__int64(SetRelativeLocation_Params) + NewLocationOffset) = *Translation;
					...
					// Pass the PARAMS struct, not the UFunction pointer (&SetPhysicsLinearVelocity was a
					// copy-paste typo — it fed the function-pointer bytes as the velocity, which would
					// fling the vehicle). The angular call below already does this correctly. (This path
					// is currently dormant — the vehicle-physics hook is disabled — but now it's correct
					// if it's ever re-enabled.)
					RootComp->ProcessEvent(SetPhysicsLinearVelocity, &SetPhysicsLinearVelocity_Params);
...
bool SpawnDefaultPawnFor(UObject* GameMode, UFunction*, void* Parameters)
{
	...
	std::cout << "[Spawn] SpawnDefaultPawnFor\n";

	// In listen-server mode the host IS a player and must get a pawn like everyone else. Only the
	// dedicated build skips the local controller (there, it isn't a participant at all) — seeing
	// this line for the SERVER's own controller is correct and expected.
	if (!Defines::bHostCanPlay && Helper::GetLocalPlayerController() == Params->PlayerController)
	{
		std::cout << "[Spawn] skipping the server's own local controller (dedicated mode)\n";
		return true;
	}
	...
	// NOTE: cosmetics are NOT applied here. Real players never reach this function — their pawn is
	// created by Helper::SpawnPawn() from ServerReadyToStartMatch, and that is where the locker
	// selection is applied. This hook only ever fires for the server's own local controller.
	return true;
}
...
bool ServerChoosePart(UObject* Pawn, UFunction*, void* Parameters)
{
	...
	// Instrumented: this RPC is how the CLIENT tells us which cosmetic parts it has equipped, and
	// it is the most likely carrier of the player's real locker selection. If these lines never
	// appear, the client isn't sending its parts at all and no server-side change can fix it.
	if (Params)
	{
		std::cout << "[Locker] ServerChoosePart type=" << (int)Params->Part.Get()
		          << " part=" << (Params->ChosenCharacterPart ? Params->ChosenCharacterPart->GetName() : "NULL")
		          << '\n';
	}
