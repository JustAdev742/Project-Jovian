...
// ── Join gate ────────────────────────────────────────────────────────────────────────────────────
// "Listening on port 7777" only means the net driver bound. The map's warmup PlayerStarts live in a
// streaming sublevel that is still loading, and a player who connects before it arrives gets no
// playerstart — ServerReadyToStartMatch then drops them at a hardcoded fallback coordinate in the air
// over the middle of the BR map instead of on the pre-match spawn island.
//
// The check runs from TickFlush, on the GAME THREAD. It must: my first attempt polled GetPlayerStart()
// from a background thread started at Listen time, which walks UObjects and calls into the engine
// before any of that is safe to touch from another thread — it faulted on a null call (0x0) and left
// the beacon paused forever, so nobody could join at all.
static bool g_WaitingForSpawnIsland = false;
static ULONGLONG g_JoinGateDeadline = 0;

/**
 * Send everyone back to the lobby before the process exits.
 *
 * A dedicated build cannot recycle in-process, so Server::Restart terminates itself at match end. If
 * the client is still connected when that happens its connection just dies, and Fortnite reports it
 * the only way it knows how: "Ack! We lost our connection to the match." Nothing was actually wrong
 * with the connection — the server was killed while they were still looking at the Victory Royale.
 *
 * Runs on the GAME THREAD from TickFlush. ClientReturnToMainMenu is a ProcessEvent call into the
 * engine, and MatchRestartThread — where the terminate lives — is a background thread, which is not
 * a safe place to make those calls from.
 */
static void SendPlayersToLobby()
{
	static auto ControllerClass = FindObject("/Script/FortniteGame.FortPlayerControllerAthena");
	static auto ClientReturnToMainMenu = FindObject<UFunction>("/Script/Engine.PlayerController.ClientReturnToMainMenu");

	if (!ControllerClass || !ClientReturnToMainMenu)
	{
		std::cout << "[Restart] cannot return players to the lobby (missing controller class or function)" << std::endl;
		return;
	}

	auto Controllers = Helper::GetAllActorsOfClass(ControllerClass);

	std::string Message = "Match complete - returning to the lobby.";
	std::wstring Wide = std::wstring(Message.begin(), Message.end());
	FString Reason;
	Reason.Set(Wide.c_str());
	...
	std::cout << "[Restart] returned " << Sent << " player(s) to the lobby before recycling" << std::endl;
}

/**
 * After a match, wait for everyone to leave — then recycle immediately.
 *
 * The host pressing "Return To Lobby" closes their own connection, which is the one kind of
 * disconnect the client does not complain about. So the moment ClientConnections empties we are done
 * and the server can go; if they sit on the results screen we keep waiting, and only fall back to
 * sending them home once the backstop expires.
 *
 * Recycling as soon as the last player leaves is also what stops a finished server lingering: the
 * agent sees the process exit, unregisters it, and starts a fresh one on the next Play.
 *
 * Game thread — reads engine state and (at the backstop) calls into the engine.
 */
static void TickMatchEndWait(UObject* NetDriver)
{
	...
	// -1 means "could not tell", which must NOT be mistaken for "nobody is connected" — that would
	// terminate the moment the match ended and put the dialog straight back.
	int Connections = -1;

	if (NetDriver)
	{
		static auto ClientConnectionsOffset = NetDriver->GetOffset("ClientConnections", false, false, false);

		if (ClientConnectionsOffset)
		{
			auto ClientConnections = (TArray<UObject*>*)(__int64(NetDriver) + ClientConnectionsOffset);
			Connections = ClientConnections->Num();
		}
	}

	// ── ABANDONED SERVER ────────────────────────────────────────────────────────────────────────
	// The host quitting mid-match never reaches EndGame, so none of the logic below would fire and
	// this process would sit there holding port 7777 with nobody in it. The launcher does clear the
	// host config when their game closes, which stops it — but only while the launcher is running.
	// This is the server taking responsibility for itself: once it has had players and then has none
	// for a sustained period, there is nothing left to host.
	static bool bHasEverHadPlayers = false;
	...
	else if (Connections == 0 && bHasEverHadPlayers && !Defines::MatchEndedAt)
	{
		if (!EmptySince)
			EmptySince = Now;
		else if (Now - EmptySince >= 60000)
		{
			// Exit HERE rather than setting bSafeToRecycle: MatchRestartThread only exists once EndGame
			// has fired, so on this path there is nobody waiting on that flag and the server would sit
			// idle forever. Terminating is what Server::Restart does for a dedicated build anyway, and
			// going straight to it avoids unwinding hooks from inside TickFlush, which is itself a hook.
			std::cout << "[Restart] everyone has disconnected and no match is finishing - shutting down"
			          << std::endl;
			std::cout.flush();
	...
	if (!Defines::MatchEndedAt)
		return; // match still in progress — nothing further to do

	if (Connections == 0)
	{
		std::cout << "[Restart] everyone has left - shutting this server down now" << std::endl;
		Defines::bSafeToRecycle = true;
		return;
	}

	const ULONGLONG Deadline = Defines::MatchEndedAt + (ULONGLONG)Defines::SecondsToWaitForPlayersToLeave * 1000ull;

	if (Now < Deadline)
		return; // still on the results screen — let them leave in their own time

	// Never left. Send them home, give the travel a moment, then go.
	static bool bSentHome = false;
	static ULONGLONG SentHomeAt = 0;

	if (!bSentHome)
	{
		std::cout << "[Restart] still " << Connections << " connected after "
		          << Defines::SecondsToWaitForPlayersToLeave << "s - sending them to the lobby" << std::endl;
		bSentHome = true;
		SentHomeAt = Now;
		SendPlayersToLobby();
	}
	else if (Now - SentHomeAt >= 5000)
	{
		Defines::bSafeToRecycle = true;
	}
}

/** Called every frame from TickFlush once a World exists. Opens the beacon when the map is ready. */
static void TickJoinGate()
{
	if (!g_WaitingForSpawnIsland)
		return;

	const ULONGLONG Now = GetTickCount64();
	const bool bTimedOut = Now >= g_JoinGateDeadline;

	// GetPlayerStart does a full actor scan of the world. Twice a second is plenty for something we
	// expect to become true within a few seconds; every frame would just burn the startup budget.
	static ULONGLONG NextCheck = 0;
	if (!bTimedOut && Now < NextCheck)
		return;
	NextCheck = Now + 500;

	if (!Helper::GetPlayerStart() && !bTimedOut)
		return;

	g_WaitingForSpawnIsland = false;

	if (bTimedOut)
		std::cout << "[Join] no spawn point in time - opening anyway" << std::endl;
	else
		std::cout << "[Join] spawn island is ready" << std::endl;

	Server::PauseBeaconRequests(false);
	std::cout << "Players may join now!" << std::endl;
}

DWORD WINAPI PauseThreadLegacy(LPVOID)
{
	Sleep(4000);
...
bool Server::Listen(int Port)
{
	...
	*Get<FName>(NetDriver, NetDriverNameOffset) = Helper::Conversion::StringToName(NetDriverNameFStr);
	...
	// Leave the beacon PAUSED and let TickJoinGate open it from the game thread once the map can
	// actually take a player. Previously this unpaused on the line right after announcing the port,
	// with nothing loaded behind it — which is how a player ended up spawning in mid-air over the
	// middle of the BR map instead of on the spawn island.
	g_WaitingForSpawnIsland = true;
	g_JoinGateDeadline = GetTickCount64() + 90000;
	std::cout << "[Join] holding the door until the spawn island is loaded" << std::endl;
...
void Server::Restart()
{
	...
	// The in-process recycle travels the LOCAL player controller to a fresh copy of the map. On a
	// dedicated server the local player is stripped, so there is no controller to travel, and the old
	// code dereferenced a null PC here — that is the access violation the auto-restart hit at match
	// end (a solo player dying is the last player -> EndMatch -> EndGame -> restart). Check for it
	// FIRST, before disabling any hooks or destroying the beacon, so a server that cannot recycle
	// in-process is left fully functional until the MatchRestartThread watchdog terminates it and the
	// launcher re-hosts a clean one.
	auto PC = Helper::GetLocalPlayerController();

	if (!PC)
	{
		// A dedicated/headless server can't recycle in-process (no local PC to travel). Previously we
		// just returned and let the ~120s watchdog terminate us — but in that window the engine's own
		// match-end flow travels WaitingPostMatch -> LeavingMap -> Frontend, and building the frontend/
		// lobby UI on a stripped build null-crashes (EXCEPTION_ACCESS_VIOLATION reading 0x90, an all-
		// game-code callstack with no Reboot frames, right after the Victory Royale). So terminate
		// cleanly RIGHT HERE instead of waiting: the launcher re-hosts a fresh server either way, and a
		// clean exit beats the engine crashing on its way back to the menu.
		std::cout << "[Restart] dedicated build - terminating now for a clean re-host "
		             "(skips the engine's post-match frontend return, which crashes a headless server).\n";
		std::cout.flush();
		TerminateProcess(GetCurrentProcess(), 0);
		return; // unreachable — process is gone
	}
...
/**
 * MCP-Profile::SendRequestNow.
 *
 * A dedicated server dispatches profile requests with a credential context it does not hold, so the
 * request is never actually sent and the client-facing symptom is:
 *
 *   "GetAthenaLoadoutWithOverrides(): MCP is enabled but we were unable to load AthenaProfile"
 *
 * ...which leaves CustomizationLoadout.Character null forever and every player wearing a random
 * skin. Forcing the context to CXC_Public (3) makes the request fire, so the game resolves the FULL
 * loadout itself — character, backpack, pickaxe, glider, contrail and variants — instead of us
 * bridging a single CID over HTTP.
 *
 * The struct field also has to be rewritten, not just the argument, because the callee re-reads it.
 * Offset differs by build: 0x28 from FN 4.2 onward, 0x60 before. (Technique from Erbium.)
 */
void* Server::Hooks::SendRequestNow(void* Arg1, void* MCPData, int ContextCredentials)
{
	if (MCPData && Engine_Version < 423)
		*(int*)(__int64(MCPData) + (Fortnite_Version >= 4.2 ? 0x28 : 0x60)) = 3; // CXC_Public

	return Defines::SendRequestNow(Arg1, MCPData, 3); // CXC_Public
}
...
void Server::Hooks::Initialize()
{
	...
	// Let the server actually read player MCP profiles (see Server::Hooks::SendRequestNow).
	// Deliberately optional: if the scan missed we run exactly as before and the HTTP locker bridge
	// still applies the player's character, so a failed scan costs full cosmetics, not a boot.
	if (SendRequestNowAddress && Defines::bForceMcpPublicContext)
	{
		std::cout << "[MCP] SendRequestNow found at 0x" << std::hex << SendRequestNowAddress << std::dec << '\n';
		std::cout << "[MCP] hook: " << MH_StatusToString(MH_CreateHook((PVOID)SendRequestNowAddress, Server::Hooks::SendRequestNow, (PVOID*)&Defines::SendRequestNow)) << '\n';
		std::cout << "[MCP] enable: " << MH_StatusToString(MH_EnableHook((PVOID)SendRequestNowAddress)) << '\n';
	}
	else
	{
		std::cout << "[MCP] context hook off (found=" << (SendRequestNowAddress ? "yes" : "no")
		          << ") - the backend locker lookup applies the player's character\n";
	}
...
void Server::Hooks::TickFlush(UObject* thisNetDriver, float DeltaSeconds)
{
	...
	if (World)
	{
		// Game thread, world exists — the only safe place for this.
		TickJoinGate();

		static auto NetDriverOffset = World->GetOffsetSlow("NetDriver");
		auto NetDriver = *Get<UObject*>(World, NetDriverOffset);

		// Needs the NetDriver to count who is still connected, so it runs after it is resolved.
		TickMatchEndWait(NetDriver);
	...
	// (A commented-out restart block used to sit here. It was a stale duplicate of Server::Restart()
	//  driven by a bShouldRestart flag nothing ever set. Removed — the live restart path is
	//  Server::Restart(), now triggered automatically at EndGame by MatchRestartThread.)

	// WARMUP HOLD. The game recomputes WarmupCountdownEndTime whenever a player joins, which would
	// shorten the lobby back to the playlist default and launch the bus early — exactly when more
	// people are arriving. Re-pin it until the hold expires, then let the game run normally.
	if (Defines::WarmupHoldUntil > 0.f)
	{
		if (auto GameState = Helper::GetGameState())
		{
			...
			if (WarmupCountdownEndTimeOffset)
			{
				...
				else
				{
					...
					// Only ever push the bus LATER, never earlier, so nothing here can cut a lobby short.
					if (*EndTime < Defines::WarmupHoldUntil)
						*EndTime = Defines::WarmupHoldUntil;

					// The storm start needs the same treatment for the same reason: a player joining
					// makes the game recompute the match timings, which would drop it back to where
					// StartMatch was called and hand the zone a head start over everyone in the lobby.
					static auto SafeZonesStartTimeOffset = GameState->GetOffset("SafeZonesStartTime", false, false, false);

					if (SafeZonesStartTimeOffset)
					{
						auto StartTime = Get<float>(GameState, SafeZonesStartTimeOffset);

						if (*StartTime < Defines::WarmupHoldUntil)
							*StartTime = Defines::WarmupHoldUntil;
					}
				}
			}
			else
			{
				Defines::WarmupHoldUntil = 0.f; // offset unavailable; don't spin on it every tick
			}
		}
	}
