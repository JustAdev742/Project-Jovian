			...
			if (BuildingActor)
			{
				...
				if (!IsBadReadPtr(MatInstance))
				{
					...
					if (!IsBadReadPtr(MatCount)) // && Helper::IsStructurallySupported(BuildingActor)
					{
						...
						// Apply the mirror flag to the spawned piece. The build path READ bMirrored but
						// only used it for the collision query, never on the actor — so a mirrored stair/
						// piece placed un-mirrored (the EDIT path threads it correctly via
						// ReplaceBuildingActor). MoonGS 7.30 sets it here. Guarded so a missing UFunction
						// is a safe no-op.
						static auto SetMirroredFn = FindObject<UFunction>("/Script/FortniteGame.BuildingSMActor.SetMirrored")
							? FindObject<UFunction>("/Script/FortniteGame.BuildingSMActor.SetMirrored")
							: FindObject<UFunction>("/Script/FortniteGame.BuildingActor.SetMirrored");
						if (SetMirroredFn)
						{
							bool bMir = bMirrored;
							BuildingActor->ProcessEvent(SetMirroredFn, &bMir);
						}
				...
				if (bShouldDestroy)
				{
					// Out of materials: destroy the spawned piece instead of leaving an un-initialized
					// ghost (no team, native-default state) sitting in the world. Both MoonGS 7.30 and
					// Reboot V3 kill the actor on a failed build. (Rarely hit — the client gates a build
					// you can't afford — but the empty branch was a real leak.)
					Helper::DestroyActor(BuildingActor);
				}
				else
				{
