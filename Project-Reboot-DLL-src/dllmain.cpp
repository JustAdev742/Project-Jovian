...
static LONG CALLBACK RebootCrashHandler(EXCEPTION_POINTERS* Info)
{
    ...
    std::cout << std::format("\n[Crash] unhandled exception 0x{:08X} at 0x{:016X} - wrote crash.log + crash.dmp\n", Code, Addr);
    std::cout.flush();
...
DWORD WINAPI Initialize(LPVOID)
{
    ...
    // Leaving a seat takes the vehicle's weapon back and returns the player's pickaxe. AddHook logs
    // and skips if the function is missing, so a build without it costs nothing.
    AddHook("/Script/FortniteGame.FortPlayerPawn.ServerOnExitVehicle", Interaction::ServerOnExitVehicle);
