...
{
	// >= not >: valid indices are 0 .. size()-1, so Index == size() is already out of bounds and
	// Items[Index] would corrupt the heap. (Also guard against a negative Index.)
	if (Index < 0 || Index >= (int)Looting::Items.size() || Index >= (int)Looting::Weights.size())
		return;
...
void Looting::Initialize()
{
	...
	// Build the loot pool ONCE. ReadyToStartMatch fires again on every match-end auto-restart, and
	// re-running the build on a re-travelled world re-reads the loot-package DataTables through
	// stale/freed row objects -> access violation at fortnite.At(i) below (this was the crash at
	// loot.cpp:972 reading 0xffffffff on the second match). The item definitions are global loaded
	// assets that survive the travel, so the pool parsed on the first match stays valid across
	// matches; reuse it instead of rebuilding.
	if (bInitialized && !Items.empty())
	{
		std::cout << "[Loot] pool already built - reusing it (skipping rebuild on match restart)\n";
		return;
	}
	...
	for (int i = 0; i < LootPackages.size(); i++)
	{
		...
		auto RowCount = fortnite.Num();

		// A stale/garbage row map (e.g. read through a freed object) can report an implausible count;
		// iterating it walks off into unmapped memory (the 0xffffffff read). Real loot tables are only
		// hundreds of rows, so treat anything outside a sane range as unusable and skip it.
		if (RowCount <= 0 || RowCount > 200000)
		{
			std::cout << "[Loot] skipping a loot package with implausible row count " << RowCount << '\n';
			continue;
		}

		if (RowCount > 0)
			bInitialized = true; // sticky: never cleared once a package has rows, so the re-init guard holds

		std::cout << "Amount of rows: " << RowCount << '\n';

		// Iterate the FULL range — `RowCount - 1` dropped the last row of every loot-package table
		// (one item variant per table could never spawn as world loot). The canonical row iterator
		// (DataTables::FindRow) uses the full range too; free/invalid rows are filtered by the
		// ComparisonIndex check below.
